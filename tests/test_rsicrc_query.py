...
def test_bi_temporal_change_calls_rsicrc():
    ...
    args, kwargs = mock_rsicrc.call_args
...
def test_vqa_intent_does_not_call_rsicrc(monkeypatch):
    ...
    assert "RSICRC" not in data["models_dispatched"]
...
def test_rsicrc_unavailable(monkeypatch):
    image_ids = upload_images(2)
    monkeypatch.setattr(settings, "MODEL_MOCK_MODE", False)
    monkeypatch.setattr(settings, "RSICRC_MOCK", False)
    monkeypatch.setattr(settings, "RSICRC_URL", "http://127.0.0.1:8001")
...
def test_rsicrc_timeout(monkeypatch):
    image_ids = upload_images(2)
    monkeypatch.setattr(settings, "MODEL_MOCK_MODE", False)
    monkeypatch.setattr(settings, "RSICRC_MOCK", False)
    monkeypatch.setattr(settings, "RSICRC_URL", "http://127.0.0.1:8001")
...
def test_rsicrc_result_is_fused_into_final_response():
    image_ids = upload_images(2)
    with patch("app.tools.models.rsicrc.rsicrc_adapter.run_rsicrc") as mock_rsicrc:
        mock_rsicrc.return_value = {
    ...
    assert data["answer"] == "Buildings expanded along the river."
    assert "Fusion decision" not in data["answer"]
    assert data["fused_evidence"].get("rsicrc", {}).get("answer") == "Buildings expanded along the river."
    assert data["execution_trace"]["selected_model"] == "RSICRC"
...
def test_upload_does_not_invoke_rsicrc():
    with patch("app.tools.models.rsicrc.rsicrc_adapter.run_rsicrc") as mock_rsicrc:
